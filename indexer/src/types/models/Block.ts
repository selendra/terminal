...
export class Block implements CompatEntity {
    ...
    constructor(
        ...
        this.evmTransactionCount = evmTransactionCount;
    ...
    public evmTransactionCount: number;
    ...
    static async getByNumber(number: number, options: GetOptions<CompatBlockProps>): Promise<Block[]> {
        // Inputs must be cast as the store interface has not been updated to support alternative ID types
        const records = await store.getByField<CompatBlockProps>('Block', 'number', number, options);
    ...
    static async getByValidatorId(validatorId: string, options: GetOptions<CompatBlockProps>): Promise<Block[]> {
        // Inputs must be cast as the store interface has not been updated to support alternative ID types
        const records = await store.getByField<CompatBlockProps>('Block', 'validatorId', validatorId, options);
        return records.map(record => this.create(record as unknown as BlockProps));
    }
